...
**للتواصل:** brainclean.app@gmail.com
...
إذا استخدمت ميزات الذكاء الاصطناعي الاختيارية — بما في ذلك دعم **صفا** السياقي، ودردشة واحة المشاعر القديمة — في نسخة إنتاج مفعَّلة سحابيًا:

- قد يُرسَل نص الرسالة التي تكتبها إلى خدمة مدعومة بالذكاء الاصطناعي عبر الشبكة.
- يُرسَل فقط ذلك النص المكتوب وأي سياق تختاره **صراحة** لذلك الإرسال.
- لا تُنقل تلقائيًا إجابات فحص الدماغ، ولا قيم ملف الذهن الكاملة، ولا تفاصيل درجة التعافي الداخلية، ولا إجابات المراجعة الأسبوعية، ولا محتوى التقارير، ولا الملاحظات الخاصة.
- تمرّ الطلبات عبر البنية الخلفية المعتمدة (**Supabase Edge Functions**) إلى مزوّدي الذكاء الاصطناعي؛ لا يوجد مسار NVIDIA، والتطبيق **لا** يضمّن مفتاح Claude مباشرًا على الجهاز.
- صفا **ليست** رعاية طبية ولا خدمات طوارئ ولا استشارات أزمات. يبقى الدعم المحلي دون اتصال متاحًا، ويمكنك رفض صفا ومتابعة استخدام بقية التطبيق.
- لا تُحفظ أرشيفات محادثات خام بشكل افتراضي.
...
- **لا إعلانات في واجهات منتج V2 الأساسية الحالية** — هذا الإصدار **لا** يعرض إعلانات؛ ولا يُباع بريميوم على أنه إزالة إعلانات في هذه المرحلة.  
